...
They are tailored towards a raiding server experience. What gets fixed is what breaks a
server or breaks the economy: crashes, item duplication, packet handlers that trust the
...
| [MFFS 3.1.0](#mffs-310) | `mergedupe` · `stabilizedupe` · `enumclamp` |
| [BalancedMFFS (coremod)](#balancedmffs-coremod) | `zones` · `logging` |
| [Mekanism 5.5.6](#mekanism-556) | `chestcrash` · `chestdupe` · `chestremote` · `machinedupe` · `robitdupe` · `tntdupe` · `tntsource` · `timeitems` · `aebridge` · `cablereload` · `pumpnodes` · `enumclamp` |
| [BalancedMekanismTimeItems (mod)](#balancedmekanismtimeitems-mod) | `stopwatch` · `weather orb` |
| [MekanismGenerators 5.5.6](#mekanismgenerators-556) | `solarspace` · `boundclear` · `metaclamp` · `particlepkt` |
| [ICBM Explosion 1.2.1](#icbm-explosion-121) | `redmatter` · `sonic` · `remote` · `explosivetype` · `empradius` · `launchertier` · `multiblock` · `cruiselauncher` · `designator` · `defuser` · `missilestack` · `listeners` · `radarradius` · `radargun` · `chunkload` · `enumclamp` |
...
| [Modular Powersuits 0.7.0](#modular-powersuits-070) | `blink` · `luxevent` · `bladeevent` · `enumclamp` |
| [MineFactoryReloaded 2.6.4](#minefactoryreloaded-264) | `ghostslot` · `pkttile` · `harvester` · `dsuside` · `rednet` · `guidupe` · `routerloop` · `dsunbt` |
| [Galacticraft a0.1.36.410](#galacticraft-a0136410) | `dimauth` · `station` · `reach` · `rider` · `guis` · `nbtclamp` · `uechunk` |
| [immibis microblocks 55.0.7](#immibis-microblocks-5507) | `dropstatic` · `placereach` |
| [NotEnoughItems 1.5.2.28](#notenoughitems-15228) | `auth` |
...
Two dupe fixes. The zone-flag and admin-logging feature that used to be baked into this jar is
now a separate coremod, [BalancedMFFS](#balancedmffs-coremod) — take the fixes without it if
you do not want the zone system.
...
![Stock MFFS: an Interdiction Matrix with an MFR conveyor belt on top and running past its side, five confiscation messages in a row for a single stack, and the duplicated items piling up on the ground](media/mffs-merge-dupe.png)

The matrix fills its 31 slots, drops the overflow, and takes the same items again on the next
pass, so the chat repeats `31 of your item(s) has been confiscated` until the belt is removed.
...
## BalancedMFFS (coremod)

A **feature**, not a bug fix, and therefore a separate jar: `BalancedMFFS.jar` goes in the
server's `coremods/` folder and transforms MFFS's own classes at load time. It works against
either the stock or the patched MFFS, and removing the jar puts MFFS straight back to stock
behaviour — the dupe fixes above stay either way.

<details>
<summary><b>Zone flags and admin logging for interdiction matrixes</b></summary>
...
Eleven fixes for crashes, dupes and packet exploits, including the Electric Chest, the Robit,
Obsidian TNT, the Stopwatch and Weather Orb, the Electric Pump, and power output to Applied
Energistics. The patcher rewrites 18 classes in `Mekanism-v5.5.6.bugfix1.jar` and adds
`VoltzMekanism` and `VoltzTimeItems`.
...
<details>
<summary><b>8. <code>timeitems</code> — set the time or weather without owning the item</b></summary>
...
It never checks that the held item is a Stopwatch, or that it has recharged, or that the
value is one the screen can actually send. Any client can change the time or the weather at
will, as often as it likes, without ever owning either item — and it damages whatever the
sender happens to be holding while doing it.

**The patch.** `VoltzTimeItems` runs first. The sender must be holding the real item, it must
be fully recharged, and the value has to be one of the four the GUI produces. Otherwise the
packet stops before the stock code touches the world, and the refusal is logged at most once
per player every 10 seconds.

That is the whole of what the patched Mekanism jar does here. What the items are *allowed* to
do once the sender genuinely owns one is a server policy, not a bug, so it lives in a separate
mod: [BalancedMekanismTimeItems](#balancedmekanismtimeitems-mod). Without it the items behave
as stock, on their stock 250-second recharge.

**Verified.** Stock: holding dirt changed the time, and a Stopwatch could be used again
instantly. Patched: both refused.

</details>

<details>
<summary><b>9. <code>pumpnodes</code> — an Electric Pump drains the whole lake after a chunk reload</b></summary>

**The bug.** The pump remembers the blocks it draws from in `recurringNodes` and comes back to
them, which is how one pump sits on one source block forever. In `suck()`, a remembered node
that is **not a liquid source at this instant** is treated as exhausted:

```java
if (isLiquid(wrapper) && sameLiquid) { take it; return true; }
for (ForgeDirection o : VALID_DIRECTIONS) {     // spread to a neighbour, within 80 blocks
    ... recurringNodes.add(side); take side; return true;
}
cleaningNodes.add(wrapper);                      // or retire it
recurringNodes.remove(wrapper);
```

`MekanismUtils.isLiquid` only counts a **source** block, metadata 0 — flowing water does not
qualify. So the moment the pump looks at its node in the gap between taking it and the source
reforming, it spreads to a neighbour and adds that to `recurringNodes`. Those nodes then fail
the same way, and the pump walks outward eating every source block within 80 blocks. If the
node lands in `cleaningNodes` instead, `clean()` then deletes the flowing water that was about
to reform it, which guarantees the same outcome.

That gap is tiny in normal running: `suck()` only fires every 20 ticks and an infinite source
reforms in a tick or two. **A chunk reload lands squarely in it** — the pump resumes on the
next world-time boundary with no relation to when it last took a block, and water physics has
not settled. Hence a pump that behaved for days suddenly draining the lake after a restart.

**The patch.** That whole path now starts with
`if (!VoltzMekanism.pumpNodeReady(this, wrapper)) continue;`, which asks the question vanilla
itself asks: a block with **two or more horizontally adjacent sources of the same liquid is
going to become a source again**. A node like that is an infinite source, so it is left alone
and the spreading never runs. A node with fewer than two really is drained, and the pump
spreads exactly as it always did, so draining a finite pool still works.

**Verified.** The harness builds the four cases against a real world on both servers: no
adjacent source and one adjacent source release the node, two adjacent sources hold it, and
removing one releases it again.

</details>

<details>
<summary><b>10. <code>aebridge</code> — Energy Cubes and Universal Cables will not power Applied Energistics</b></summary>

**The bug.** Mekanism can push power into a BuildCraft `IPowerReceptor`, and Applied
Energistics accepts power through exactly that interface. But Mekanism gates the whole output
path on `Loader.isModLoaded("BuildCraft|Energy")`. Voltz has no BuildCraft — AE just bundles
the BuildCraft power **API** — so the gate is false, the output path is skipped, and an Energy
Cube or Universal Cable placed against an ME machine transfers nothing. It reads as AE
"ignoring" Mekanism power.

**The patch.** `CableUtils` and `TileEntityEnergyCube` ask `VoltzMekanism.bcPowerAvailable()`
instead of the mod loader. That returns true when the BuildCraft power API classes are on the
classpath at all, which is what the code actually needs, so AE (or anything else shipping the
API) receives power. With a real BuildCraft install the answer is the same as before.

</details>

<details>
<summary><b>11. <code>cablereload</code> — an AE link dies when the chunk reloads</b></summary>

**The bug.** `EnergyNetwork` caches its acceptor list. A tile that leaves and comes back —
a chunk unload and reload, the usual case after a restart — is a different object, and the
cached list still holds the dead one, so the cable silently stops delivering until the network
is disturbed by hand.

**The patch.** The network rescans its acceptors on a timer (once every 60 ticks per network,
tracked in `VoltzMekanism.shouldRefreshNetwork`), so a reloaded tile is picked up on its own.

</details>

---

## BalancedMekanismTimeItems (mod)

A **feature**, not a bug fix, and therefore a separate jar: `BalancedMekanismTimeItems.jar`
goes in the server's `mods/` folder next to the patched Mekanism. It transforms nothing, so it
is an ordinary mod rather than a coremod — the patched Mekanism looks it up by name and hands
it the decision when it is present. Leave it out and the Stopwatch and Weather Orb behave as
stock, still covered by the [`timeitems`](#mekanism-556) exploit fix.

<details>
<summary><b>Server policy for the Stopwatch and Weather Orb</b></summary>

Anti-griefing aside, a Stopwatch in every player's pocket means the server's time is whatever
the last person to click decided, which most operators want some say over. The exploit fix in
the patched Mekanism jar has already established that the sender is holding the real item and
that it is recharged; this decides what that click is allowed to do:
...
**Verified.** `disable` and `command` modes behaved as above, and on MCPC+ the command ran
through Bukkit (`* VTest voted for noon via VTest`). The hand-off between the two jars is
checked on every harness run, on both plain Forge and MCPC+, because a typo in the name the
fix looks the mod up by would leave the policy silently switched off.
...
Four fixes: a multiblock that deletes whatever is above it, a teardown that deletes its
neighbours, a chunk-load crash from a bad metadata, and a packet that loads chunks.

<details>
<summary><b>1. <code>solarspace</code> — placing an Advanced Solar Generator deletes the block above it</b></summary>

**The bug.** `TileEntityAdvancedSolarGenerator.onPlace()` claims ten positions: the block
directly above the generator, and the 3x3 two above it. Each is overwritten with
`world.setBlock(..., BoundingBlock)` — no check, no drop. `ItemBlockGenerator.placeBlockAt`
is supposed to refuse the placement when that space is occupied, and it does check the 3x3,
but it **never checks the single block at y+1**. Place an Advanced Solar Generator under a
chest, a machine, or anyone's build and that block and everything inside it is gone.

The Wind Turbine checks all four of its positions, so only the Advanced Solar Generator is
affected.

**The patch.** `placeBlockAt` starts with `VoltzMekGen.canPlace`, which requires every
position the multiblock is about to claim to be free — including y+1 — for both tall
generators, and refuses the placement otherwise.

</details>

<details>
<summary><b>2. <code>boundclear</code> — breaking a multiblock deletes whatever replaced its bounding blocks</b></summary>

**The bug.** `onBreak()` blanks each position the multiblock claimed with `setBlockToAir`,
whether or not that position still holds one of its own bounding blocks.

**The patch.** Every one of those calls goes through `VoltzMekGen.clearBound`, which only
clears a position still holding a Mekanism bounding block or the generator itself.

</details>

<details>
<summary><b>3. <code>metaclamp</code> — a bad metadata fails the chunk, permanently</b></summary>

**The bug.** `GeneratorType.getFromMetadata(meta)` is `values()[meta]` with no bounds check,
and `createTileEntity` calls it while the chunk is loading. A generator block carrying a
metadata above the last generator type throws out of chunk load — and it throws again every
time that chunk is read.

**The patch.** The argument is clamped to the enum's real length first.

</details>

<details>
<summary><b>4. <code>particlepkt</code> — an Electrolytic Separator packet loads any chunk</b></summary>

**The bug.** `PacketElectrolyticSeparatorParticle.read` calls `getBlockTileEntity` on
coordinates straight off the wire. Spawning smoke is a no-op on a server, but the lookup
loads — or generates — whatever chunk the client named.

**The patch.** The lookup goes through `VoltzMekGen.loadedTile`, which answers only for
...
<details>
<summary><b>1. <code>blink</code> — Blink Drive leaves you inside walls</b></summary>
...
The same patch also gates the move on a `PlayerTeleportEvent`. Stock calls
`setPositionAndUpdate` directly, which is not CraftBukkit's teleport path, so region entry
rules never saw a Blink; now they do, a cancellation stops the teleport, and a plugin that
redirects the destination is honoured. Range is unchanged (5m base, 59m at most) and the move
is still a ray trace that stops at the first block, so it cannot pass through bedrock.
...
<details>
<summary><b>2. <code>luxevent</code> / <code>bladeevent</code> — two items change the world where Bukkit cannot see them</b></summary>

**The bug.** Neither of these is an exploit in the mod's own terms; both are invisible to
Bukkit, so on MCPC+ no protection plugin can act on them.

The **Lux Capacitor** is thrown, flies flat (`getGravityVelocity` returns 0) for 400 ticks at
one block per tick, and on impact does:

```java
world.setBlock(x, y, z, BlockLuxCapacitor.assignedBlockID, 0, 7);
world.setBlockTileEntity(x, y, z, new TileEntityLuxCapacitor(dir));
```

No player, no event. That plants a block up to a few hundred blocks away, inside any claim,
and WorldGuard never hears about it. The thrower is recorded on the entity all along — stock
simply never uses it.

The **Blade Launcher**'s blade shears leaves and vines and then calls
`world.destroyBlock(x, y, z, true)`, which is not the player-break path, so no
`BlockBreakEvent` fires either.

**The patch.** The placement goes through `VoltzMPS.luxPlace`, which fires a `BlockPlaceEvent`
with the thrower as the player and does not place the block if it is cancelled or
`canBuild` is false; `luxTile` then only attaches the tile entity if the block really went
down. The blade's shear and destroy are decided together in `bladeMayShear` — once, because
stock spawns the shear drops *before* it destroys the block, so refusing only the destroy
would hand out the shears for free — and both honour a `BlockBreakEvent`.
...
Plasma Cannon and Active Camouflage were also decompiled and checked for crashes, dupes and
exploits. None turned up — they work as designed, so they are not patched. Whether to allow
them is a balance call for each server.

## MineFactoryReloaded 2.6.4

Eight fixes: a packet handler that trusts the client completely, a RedNet Logic packet that
instantiates arbitrary classes, three GUIs that outlive their machine, a router loop that
crashes the server thread, and a Deep Storage Unit hand-off that leaks contents between blocks.

<details>
<summary><b>1. <code>ghostslot</code> — copy your held item into any inventory, or empty any slot, from anywhere</b></summary>
...
helper passes a mock test of the legitimate write and of each refusal case, and the runtime
harness exercises it on both plain Forge and MCPC+.

</details>

<details>
<summary><b>2. <code>pkttile</code> — every other MFR packet acts on any machine in the world</b></summary>

**The bug.** `ServerPacketHandler.onPacketData` handles eleven more packets, and each one
begins the same way:
...
with `x`, `y`, `z` read straight off the wire — no reach test, no check that the player has
that machine's GUI open, and `getBlockTileEntity` loads or generates the chunk it lands in.
A modified client can name any coordinates in the world and:

| Packet | What it does to a stranger's machine |
|---|---|
| 2 | retune an Auto-Enchanter, or change a Block Smasher's fortune |
| 3 | write a Harvester setting |
| 4 | flip a Chronotyper |
| 5 | flip a Deep Storage Unit's output sides — which is how you point a pipe at someone's DSU and drain it |
| 9 | play, stop, or **copy** the record in an Auto Jukebox |
| 10 | flip an Auto-Spawner's exact-copy mode |
| 13/14/15/16 | reprogram a RedNet Logic block |
| 17 | flip an Enchantment Router or Item Router |

Every one of them also forces a chunk load at arbitrary coordinates, which is a cheap way to
grind a server to a halt.

**The patch.** All twelve lookups (the eleven above plus packet 19's) go through
`VoltzMFR.tileAt`, which answers only for a tile in an already-loaded chunk within the
player's reach. When it refuses, the packet's `instanceof` fails and the handler stops on
its own, and the refusal is logged at most once per player every 10 seconds.

</details>

<details>
<summary><b>3. <code>rednet</code> — a RedNet Logic packet instantiates any class on the server</b></summary>

**The bug.** Packet 14 carries a circuit number and a **class name**, and the tile does:

```java
private IRedNetLogicCircuit getNewCircuit(String s) {
    try { return (IRedNetLogicCircuit) Class.forName(s).newInstance(); }
    catch (Exception e) { e.printStackTrace(); return new Noop(); }
}
```

The cast is checked *after* `newInstance()` has already run the constructor, so a client can
have the server construct any class on its classpath that has a public no-argument
constructor, and the failure is swallowed as a stack trace. On top of that the circuit
number, pin number and buffer number in packets 13, 14 and 15 are used as raw array
subscripts — `_circuits[n]`, `_pinMappingInputs[n][n2]`, `_buffers[b][i]` — so an
out-of-range value throws straight out of the packet handler.

**The patch.** Those four entry points go through `VoltzMFR.rnSendDef`, `rnInit`, `rnPinIn`
and `rnPinOut`. Each bounds-checks its indices against the tile's real arrays first, and
`rnInit` only accepts a class name belonging to a circuit that was actually registered
through `MFRRegistry.registerRedNetLogicCircuit` — all 67 of MFR's own circuits, and
anything another mod registers, so the GUI's circuit list still works unchanged.

</details>

<details>
<summary><b>4. <code>harvester</code> — a packet grows a Harvester's settings without limit</b></summary>

**The bug.** Packet 3 does `harvester.getSettings().put(clientString, clientBoolean)`. The
tile defines exactly three keys (`silkTouch`, `harvestSmallMushrooms`, `harvestJungleWood`)
and writes the whole map to NBT on every world save, so a client can grow that map — and the
region file holding it — as far as it likes.

**The patch.** The write goes through `VoltzMFR.harvesterSetting`, which accepts only those
three keys.

</details>

<details>
<summary><b>5. <code>dsuside</code> — a Deep Storage Unit side index throws out of the handler</b></summary>

**The bug.** Packet 5 uses the client's side number directly on a six-element array, in both
`getIsSideOutput` and `setSideIsOutput`. Anything outside 0..5 throws.

**The patch.** Both calls go through `VoltzMFR.dsuGetSide` / `dsuSide`, which ignore an index
outside 0..5.

</details>

<details>
<summary><b>6. <code>guidupe</code> — three MFR GUIs outlive their machine</b></summary>

**The bug.** `TileEntityFactoryInventory.isUseableByPlayer` does the right thing — it checks
that the tile is still the one at its own position before allowing the container to stay
open. Three tiles override it and drop that half, keeping only the distance test:

```java
public boolean isUseableByPlayer(EntityPlayer p) {
    return p.getDistanceSq(xCoord, yCoord, zCoord) <= 64.0;
}
```

They are the **Deep Storage Unit**, the **Laser Drill** and the **LiquiCrafter**. Break one
with its GUI open and the block drops its contents while the container still holds a live
reference to the orphaned tile, so the slots can be emptied a second time. The Deep Storage
Unit is the worst of the three: its drop carries the whole stored quantity in NBT and slot 2
still holds up to a full stack.

**The patch.** All three delegate to `VoltzMFR.tileUsable`, which restores the base class's
semantics: the tile must still be the one the world has at its own position, and the player
must be within reach.

</details>

<details>
<summary><b>7. <code>routerloop</code> — two Liquid Routers pointed at each other kill the server thread</b></summary>

**The bug.** `TileEntityLiquidRouter.fill` routes straight into its neighbours' `fill`:

```java
public int fill(ForgeDirection d, LiquidStack l, boolean doFill) { return pumpLiquid(l, doFill); }
// pumpLiquid -> weightedRouteLiquid -> ((ITankContainer) neighbour).fill(...)
```

Two routers aimed at each other — or any ring of them — recurse until the server thread runs
out of stack. A `StackOverflowError` inside a world tick takes the server down.

**The patch.** Both neighbour calls go through `VoltzMFR.routeFill`, which caps the chain at
8 hops per thread. That is deeper than any real routing setup and the recursion simply stops
instead of crashing. Verified live: a tank that routes back into itself terminates after 8
hops.

</details>

<details>
<summary><b>8. <code>dsunbt</code> — a broken Deep Storage Unit hands its contents to the next one</b></summary>

**The bug.** A Deep Storage Unit does not drop its items when broken. `BlockFactoryMachine`
stashes the whole tile's NBT in a static map, and `getBlockDropped` attaches it to the block
item a moment later:

```java
BlockNBTManager.setForBlock(tileEntity);           // in breakBlock
itemStack.setTagCompound(BlockNBTManager.getForBlock(x, y, z));   // in getBlockDropped
```

The map is keyed by x/y/z alone — **no dimension, no expiry**. Break a full DSU in a way that
never reaches `getBlockDropped` (creative, an explosion, a block breaker) and the entry sits
there forever: the next DSU broken at those coordinates, in any dimension, drops carrying the
old one's contents.

**The patch.** Both calls go through `VoltzMFR.setBlockNbt` / `getBlockNbt`, which keep one
pending hand-off **per world thread**, valid only for the coordinates it was written for and
only for the tick it was written in. That is all the stock path ever needed — the write and
the read happen in the same tick on the same thread — and a stale entry can no longer be
handed to a later block. Being per thread, it also cannot be crossed between MCPC+'s
per-dimension world threads.

</details>

---

## Galacticraft a0.1.36.410

Galacticraft is a **coremod**: the patched jar goes in the server's `coremods/` folder, not
`mods/`. Seven fixes, all in packet handling.

<details>
<summary><b>1. <code>dimauth</code> — teleport to any dimension</b></summary>

**The bug.** Packet 2 passes the client's dimension id to
`WorldUtil.transferEntityToDimension` with no check that the player can reach it.

**The patch.** Both calls go through `VoltzGC.transferToDimension`, which refuses a
destination that is not in the player's own list of reachable dimensions.

</details>

<details>
<summary><b>2. <code>station</code> — a free space station</b></summary>

**The bug.** Packet 15 binds a new space station dimension to the sender without confirming
the recipe matches, so the station is created whether or not the materials are there.

**The patch.** `VoltzGC.createSpaceStation` checks `SpaceStationRecipe.matches` before
consuming the materials and binding the dimension.

</details>

<details>
<summary><b>3. <code>reach</code> — three packets act on any machine in the world</b></summary>

**The bug.** Packets 11, 17 and 22 act on the tile at coordinates read off the wire, with no
reach test and no loaded-chunk test.

**The patch.** All three are gated on `VoltzGC.reachPacket`: the chunk must already be loaded
and the position within the player's reach.

</details>

<details>
<summary><b>4. <code>rider</code> — move any controllable entity</b></summary>

**The bug.** `GCCorePacketEntityUpdate` moves the entity the client names, whether or not the
sender is riding it.

**The patch.** `VoltzGC.entityIfRider` keeps the found entity only when the sender is actually
its rider.

</details>

<details>
<summary><b>5. <code>guis</code> — six machine GUIs never check reach</b></summary>

**The bug.** Six Galacticraft machine containers return `true` from `canInteractWith`
unconditionally, so the GUI outlives the machine.

**The patch.** Each delegates to `VoltzGC.containerReach`, which defers to the backing tile's
own reach test.

</details>

<details>
<summary><b>6. <code>nbtclamp</code> — a corrupt save fails the chunk</b></summary>

**The bug.** `GCCoreTileEntityParachest` and `GCCoreEntityLander` size their chest arrays from
a length read out of NBT, so a corrupt or hand-edited save throws during load.

**The patch.** The length is clamped to a sane range first.

</details>

<details>
<summary><b>7. <code>uechunk</code> — bundled Universal Electricity packets load any chunk</b></summary>

**The bug.** The Universal Electricity `PacketManager` bundled inside Galacticraft dispatches
tile packets by client coordinates, loading whatever chunk they name.

**The patch.** The dispatch is gated on the chunk already being loaded.

</details>

**Note.** The Parachest's adjacency behaviour was examined and is **not** a dupe — it was
...
Both `immibis-microblocks-55.0.7.jar` (coremods) and `immibis-core-55.1.6.jar` (mods) ship the
same `BlockMultipartBase`, and either copy can win the class load, so **both jars are patched**.

<details>
<summary><b>1. <code>placereach</code> — place a microblock anywhere in the world</b></summary>

**The bug.** `PacketMicroblockPlace.onReceived` bounds-checks the position enum and then
places the held microblock at whatever coordinates the client sent — no reach test, no
loaded-chunk test. The method it calls is named `placeInBlockWithBukkitEvent`, but in this
build it fires no event at all: `Microblocks_MCPC_Compat.isMCPC()` returns a hardcoded
`false`.

**The patch.** The call goes through `VoltzMicro.placeGuarded`, which requires the target to
be in a loaded chunk within the player's reach before the placement runs.

</details>

<details>
<summary><b>2. <code>dropstatic</code> — two dimensions can swap each other's part drops</b></summary>

**The bug.** Breaking a multipart stashes the drop in a single static field that vanilla
picks up from `getBlockDropped` a moment later:

```java
private static List lastDrop = null;
```

MCPC+ ticks each dimension on its own thread. Two players breaking parts in different
dimensions at the same moment can have one hand-off overwrite the other: one gets both drops,
the other gets none.

**The patch.** Every read and write of that field goes through `VoltzMicro.getLastDrop` /
`setLastDrop`, which keep it per thread. Verified live: a value set on one thread is not
...
NEI is a **coremod**: the patched jar goes in the server's `coremods/` folder.

<details>
<summary><b><code>auth</code> — two server packets have no permission check at all</b></summary>

**The bug.** `NEIServerConfig.authenticatePacket` is NEI's permission gate, and it is a
`switch` that names a feature for most of NEI's server packets — give, delete, creative,
enchant, potion, time, rain, magnet — and then falls through to `return true` for everything
it does not list. Two packets fall through:

- **packet 15** retunes the mob spawner at whatever coordinates the client names, to whatever
  mob name it sends. Any player, any distance, any spawner in the world, and the lookup loads
  the chunk it names.
- **packet 25** writes a dummy slot in the sender's open container.

The gated features themselves are fine — every one of them defaults to `OP` in
`config/NEIServer.cfg`, so give and creative mode are locked down out of the box. The gap is
only in what the switch never listed.

**The patch.** The stock method keeps its body under a private name and a wrapper hands its
verdict to `VoltzNEI.auth`, which passes a refusal straight through, leaves an already-gated
packet alone, and requires the `creative` permission for packets 15 and 25. A refusal is
logged at most once every 10 seconds:

```
[VoltzFixes] refused NEI packet 15 from <player>
```
...
## enumclamp (Mekanism, MFFS, Modular Powersuits, ICBM Explosion)

<details>
<summary><b><code>enumclamp</code> — one bad ordinal makes a chunk permanently unloadable</b></summary>

**The bug.** Four mods index an enum's `values()` with a number that came straight out of NBT
or block metadata, with no bound:

| Mod | Site | Reached from |
|---|---|---|
| Mekanism | `BlockMachine$MachineType.getFromMetadata` — `values()[meta]` | `createTileEntity`, i.e. chunk load |
| MFFS | `TileEntityFortronCapacitor.readFromNBT` — `values()[nbt.getInteger("transferMode")]` | chunk load |
| Modular Powersuits | `TileEntityLuxCapacitor.readFromNBT` — `values()[nbt.getInteger("s")]` | chunk load, and `onDataPacket` |
| ICBM Explosion | `EDaoDan.readEntityFromNBT` — `values()[nbt.getInteger("xingShi")]` | entity load |
...
that chunk is read**. One bad value and the chunk never loads again. The MPS one is also
reachable from a tile-entity data packet, and the ICBM one costs a missile rather than a
chunk, because 1.5.2 catches entity load failures.

The same bug in MekanismGenerators is the `metaclamp` patch above; this is the rest of them.

**The patch.** Each site becomes `VoltzX.enumAt(values(), index)`, which returns the first
constant and logs once when the ordinal is out of range. The setting is lost, the world is
not. The patcher reads the element type back out of the `values()` descriptor rather than
hard-coding enum names, and asserts it found exactly one site per class.

**Verified.** Each helper is exercised in range, past the end, negative and against an empty
array, on both servers, and `MachineType.getFromMetadata(99)` and `(-1)` are called for real
and no longer throw while `(0)` is unchanged.
...
Override paths with `MODS`, `COREMODS`, `AS_SRC`, `MPSA_SRC`, `MFFS_SRC`, `MEK_SRC`, `MEKGEN_SRC`,
`ICBM_SRC`, `ICBMS_SRC`, `ICBMC_SRC`, `MPS_SRC`, `MFR_SRC`, `GC_SRC`, `MICRO_SRC`, `ICORE_SRC`,
`NEI_SRC`, `FORGE`, `ASM`, `JAVAC8`.
...
which are built from `src/` rather than patched out of a mod jar.
...
| `MFFS_v3.1.0.175.jar` | `MFFS_v3.1.0.175-patched.jar` |
...
| `MekanismGenerators-v5.5.6.bugfix1.jar` | `MekanismGenerators-v5.5.6.bugfix1-patched.jar` |
| `immibis-core-55.1.6.jar` | `immibis-core-55.1.6-patched.jar` |

Three of them are **coremods** and belong in the server's `coremods/` folder instead:

| Replace | With |
|---|---|
| `Galacticraft-1.5.2-a0.1.36.410.jar` | `Galacticraft-1.5.2-a0.1.36.410-patched.jar` |
| `immibis-microblocks-55.0.7.jar` | `immibis-microblocks-55.0.7-patched.jar` |
| `NotEnoughItems 1.5.2.28.jar` | `NotEnoughItems 1.5.2.28-patched.jar` |

`BalancedMFFS.jar` and `VoltzLoginGuard.jar` are coremods too, and are additions rather than
replacements. `BalancedMFFS.jar` is the optional zone-flag and admin-logging feature; leave it
out and MFFS behaves exactly as stock apart from the two dupe fixes.

`BalancedMekanismTimeItems.jar` is an ordinary mod and goes in `mods/`, also as an addition.
It is the optional Stopwatch and Weather Orb policy; leave it out and those two items behave
as stock, still covered by the `timeitems` exploit fix in the patched Mekanism jar.
...
Mekanism-v5.5.6.bugfix1-patched.jar
  added   mekanism/common/VoltzMekanism.class (and 2 inner classes)
          mekanism/common/VoltzTimeItems.class
  changed mekanism/common/TileEntityElectricChest.class      slot list, no nesting
          ...
          mekanism/common/network/PacketTime.class           held item + recharge check
          mekanism/common/network/PacketWeather.class        held item + recharge check
          mekanism/common/CableUtils.class                   BC power API, not the BC mod
          mekanism/common/TileEntityEnergyCube.class         BC power API, not the BC mod
          mekanism/common/EnergyNetwork.class                periodic acceptor rescan
